...
function iris_wp_register_components($components) {
...
Hooks::prefixed_add_filter('components', 'iris_wp_register_components', 20);
Theme::load(
    array(
 ...
 * @return Theme
 */
function iris_wp_theme() {
    return Theme::getInstance();
}

function iris_wp_assets() {
    return iris_wp_theme()->getAssetsManager();
}
...
if (!apply_filters('kubio_is_enabled', false)) {
    iris_wp_assets()
        ...
        ->registerStylesheet('iris-wp-theme', '/theme/theme.css');
    iris_wp_assets()->loadLocalGoogleFonts();
    //	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
    //		iris_wp_assets()->addGoogleFont( $family, $variants );
    //	}

    add_action('wp_enqueue_scripts', 'iris_wp_print_color_scheme', 0);
} else {
    iris_wp_assets()
        ->registerStyle('iris-wp-theme', iris_wp_assets()->getBaseURL() . '/theme/fse-base-style.css');
    Hooks::prefixed_add_filter('skip_google_fonts', '__return_true');
}

function iris_wp_theme_add_woocommerce_support() {
    add_theme_support(
        'woocommerce',
        array(
            'product_grid' => array(
...
add_action('after_setup_theme', 'iris_wp_theme_add_woocommerce_support');
...
function iris_wp_get_builder_plugin_slug() {
...
function iris_wp_theme_plugins($plugins) {

    $plugin_slug = iris_wp_get_builder_plugin_slug();

    $plugins[$plugin_slug] = array(
        'name'        => $plugin_slug === 'kubio-pro' ? __('Kubio PRO', 'iris-wp') : __('Kubio', 'iris-wp'),
...
Hooks::prefixed_add_filter('theme_plugins', 'iris_wp_theme_plugins');

Hooks::prefixed_add_action(
    'after_plugin_activated',
    function ($slug) {

        if ($slug === iris_wp_get_builder_plugin_slug()) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
                            ...
                            echo esc_html(
                                sprintf(
                                    __('%s design has been successfully imported!', 'iris-wp'),
                                    iris_wp_theme()->getName()
                                )
                            );
                            ?>
                        </p>
                        <p class="imported-subtitle">
                            ...
                            echo esc_html(
                                sprintf(
                                    __('%s design has been successfully imported! You can take a look at your new design or start editing it', 'iris-wp'),
                                    iris_wp_theme()->getName()
                                )
                            );
                            ?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url(site_url()); ?>">
                            <?php echo esc_html(__('View site', 'iris-wp')); ?>
                        </a>
...
function iris_wp_try_online_url() {
    return "https://kubiobuilder.com/go/try-theme/" . get_template();
}

add_filter('kubio/editor-try-online/url', 'iris_wp_try_online_url', 10);

function iris_wp_render_header_style() {

    ?>
    <base target="_top">
    <?php

    iris_wp_theme()->get('css')->render();
}
add_action('wp_head', 'iris_wp_render_header_style', 100);
