...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".iris-wp-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.iris_wp_admin) {
    return;
  }

  var _window$iris_wp_admin = window.iris_wp_admin,
      builderStatusData = _window$iris_wp_admin.builderStatusData,
      getStartedData = _window$iris_wp_admin.getStartedData;
  ...
  function disableNotice() {
    ...
    $notice_container.closest(".iris-wp-admin-big-notice").find("button.notice-dismiss").click();
  }

  function toggleProcessing(value) {
    $(window).on("beforeunload.iris-wp-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".iris-wp-admin-big-notice").addClass("processing");
      $(".iris-wp-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".iris-wp-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.iris-wp-admin-big-notice");
    });
  }
  ...
  $notice_container.closest('.iris-wp-admin-big-notice').on("click", ".iris-wp-notice-dont-show-container", disableNotice);
  ...
  window.iris_wp_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
